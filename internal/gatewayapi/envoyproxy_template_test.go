...
//
// Test cases are defined as YAML files in testdata/envoyproxy-template/ directory:
// - <test-name>.in.yaml: Input containing envoyGateway config (with template), gatewayClass, gateway, and optional envoyProxies
// - <test-name>.out.yaml: Expected merged EnvoyProxy output
func TestEnvoyProxyTemplate(t *testing.T) {
	// Find all test input files
	inputFiles, err := filepath.Glob(filepath.Join("testdata", "envoyproxy-template", "*.in.yaml"))
	require.NoError(t, err)
	require.NotEmpty(t, inputFiles, "No test input files found in testdata/envoyproxy-template/")
	...
	for _, inputFile := range inputFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			outputFile := filepath.Join("testdata", "envoyproxy-template", testName+".out.yaml")
...
// This structure matches the YAML input files in testdata/envoyproxy-template/
